##########################################################################################################
# Created by Nitish Gupta 28-June-2019
# Requested by EMDS OAIMS for Auto code deployment
# This is a script to migrate APEX files as part of the Auto code deployment process.
##########################################################################################################
...
if [ "${FNAME}" == *00.sql* ]; then 
APEX_HEADER=`echo $file |cut -b 1,2,3,4,5`
APEX_FILE_NAME="$APEX_HEADERheader"
else
echo "Object Type is not matching. File migrate_apex.sh can only be used with APEX objects. Please check."
if
...
AXHF="/u05/jenkins/instances/eo-jenkins/jenkins_data/oaims_code_migration_scripts/apex_header_files"
...
SCRIPT="$SPATH/$FNAME"
echo $SCRIPT
SCRIPT_APEX="$AXHF/$APEX_FILE_NAME"
